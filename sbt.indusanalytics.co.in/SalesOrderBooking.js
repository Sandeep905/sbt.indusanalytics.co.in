...
var SalesManagerId = 0, ClientCordinatorID = 0, SalesCordinatorId = 0;
var QuotationNo = '';
...
$("#SOBProductData").dxDataGrid({
    ...
    onSelectionChanged: function (data) {
        ...
        $("#SOBSalesRep").dxSelectBox({
            value: data.selectedRowsData[0].SalesPersonID,
        });
        $("#SOBClientName").dxSelectBox({
    ...
    try {
        ...
        for (var i = 0; i < grid._options.dataSource.length; i++) {
            ...
            if (Number(grid._options.dataSource[i].CGSTAmount) > 0) {
                SaveOrder.CGSTTaxPercentage = grid._options.dataSource[i].CGSTPercantage;
                SaveOrder.CGSTTaxAmount = grid._options.dataSource[i].CGSTAmount;
            }
            if (Number(grid._options.dataSource[i].SGSTAmount) > 0) {
                SaveOrder.SGSTTaxPercentage = grid._options.dataSource[i].SGSTPercantage;
                SaveOrder.SGSTTaxAmount = grid._options.dataSource[i].SGSTAmount;
            }
            if (Number(grid._options.dataSource[i].IGSTAmount) > 0) {
                SaveOrder.IGSTTaxPercentage = grid._options.dataSource[i].IGSTPercantage;
                SaveOrder.IGSTTaxAmount = grid._options.dataSource[i].IGSTAmount;
            }
            SaveOrder.NetAmount = grid._options.dataSource[i].Amount;
