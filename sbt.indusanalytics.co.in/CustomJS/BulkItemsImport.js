...
var MasterID = 0, MasterName = "", GblItemNameString = "", GblItemDecString = "";
...
try {
    ...
    $.ajax({
        ...
        success: function (results) {
            ...
            $("#SelItemGroup").dxSelectBox({ items: RES1 });
        }
    });
} catch (e) {
    //  $("#LoadIndicator").dxLoadPanel("instance").option("visible", false);
...
function SaveData() {
    try {
        ...
        MasterName = $("#SelItemGroup").dxSelectBox("instance").option('text');
        ...
        for (var i = 0; i < excelRows.length; i++) {
            ...
            for (var j = 0; j < masterColumns.length; j++) {
                ...
                ObjDetailsData[ArrIMDRecord.FieldName] = ArrIMDRecord.FieldValue; //for main table
            ...
            $.ajax({
                ...
                data: '{CostingDataItemMaster:' + JSON.stringify(ObjMainData) + ',CostingDataItemDetailMaster:' + JSON.stringify(ObjIMDRecord) + ',MasterName:' + JSON.stringify(MasterName) + ',ItemGroupID:' + JSON.stringify(MasterID) + ',ActiveItem:' + JSON.stringify(true) + '}',
                contentType: "application/json; charset=utf-8",
                dataType: "json",
                success: function (results) {
