...
$("#SelIsExport").dxSelectBox({
    ...
    onValueChanged: function (e) {
        if (e.value === "No") {
            ...
            $("#DivIsExportRemarks").addClass("hidden");
        } else {
            $("#DivExportOptionNo").addClass("hidden");
            $("#DivExportOptionYes").removeClass("hidden");
            $("#DivIsExportRemarks").removeClass("hidden");
        }
    }
}).dxValidator({
...
$("#DivNotifyParty2").dxRadioGroup({
    items: ["Notify Pary 2", "Delivery Address"],
    layout: "horizontal"
});
...
var DeliveryDetailGrid = $("#DeliveryDetailGrid").dxDataGrid({
    ...
    onEditingStart: function (e) {
        if (e.column.dataField === "PurchaseRate" || e.column.dataField === "PurchaseUnit") {
            ...
            if (IsDirectInvoice === false)
                e.cancel = true;
        } else if (e.column.dataField === "PackingReference" || e.column.dataField === "NoOfPallet" || e.column.dataField === "IssueOuterCarton" || e.column.dataField === "InnerCarton" || e.column.dataField === "PONo" || e.column.dataField === "PODate") {
            if (IsDirectInvoice === false)
                e.cancel = true;
        }
    },
    ...
    columns: [
        ...
        { dataField: "FreeQuantity", allowEditing: true, caption: "Free Qty" },
        ...
        { dataField: "PurchaseRate", allowEditing: true, caption: "Rate", validationRules: [{ type: "required" }, { type: "numeric" }] },//Rate
        {
            ...
            lookup: {
                dataSource: [{ "PurchaseUnit": "UnitCost", "PurchaseUnitX": "Per Unit" }, { "PurchaseUnit": "UnitThCost", "PurchaseUnitX": "1000 Unit" }, { "PurchaseUnit": "UnitThCost", "PurchaseUnitX": "1000 Unit" }, { "PurchaseUnit": "Numbers", "PurchaseUnitX": "Numbers" }],
        ...
        { dataField: "NoOfPallet", allowEditing: true, caption: "No Of Pallet" },
        { dataField: "IssueOuterCarton", allowEditing: true, caption: "No Of Carton" },
        { dataField: "InnerCarton", allowEditing: true, caption: "Unit Per Carton" },
        { dataField: "NetWeight", allowEditing: true, caption: "Net Weight" },
        { dataField: "GrossWeight", allowEditing: true, caption: "Gross Weight" },
        ...
        { dataField: "PONo", allowEditing: true, caption: "PO No" },
        { dataField: "PODate", allowEditing: true, caption: "PODate", dataType: "date", format: "dd/MM/yyyy", Mode: "DateRangeCalendar" },
...
$("#BtnCreateInvoice").click(function () {
    ...
    CreateNOTENO();
    $("#TxtClientID").dxSelectBox({ value: GetSelectedListData[0].LedgerID });
            ...
            if (ObjOrderDeliveryData.length > 0) {
                ...
                if (ObjOrderDeliveryData.length > 0) {
                    ...
                    for (var CreatePOGridRow = 0; CreatePOGridRow < ObjOrderDeliveryData.length; CreatePOGridRow++) {
                        ...
                        ObjOrderDeliveryData[CreatePOGridRow].DiscountAmount = DiscountAmt;
...
$("#BtnSave").click(function () {
    ...
    var OptDivNotifyParty2 = $("#DivNotifyParty2").dxRadioGroup('instance').option('value');
    ...
    OperationInvoicemain.NotifyParty2Caption = OptDivNotifyParty2;
    ...
    OperationInvoicemain.TotalDistance = TxtDistance;
    ...
    for (var h = 0; h < DeliveryDetailGridRow; h++) {
        ...
        OperationInvoiceDetyail.DiscountAmount = Number(DeliveryDetailGrid._options.dataSource[h].DiscountAmount).toFixed(2);// (Number(DeliveryDetailGrid._options.dataSource[h].BasicAmount) - Number(DeliveryDetailGrid._options.dataSource[h].AfterDisAmt)).toFixed(2);
        ...
        OperationInvoiceDetyail.IsService = DeliveryDetailGrid._options.dataSource[h].IsService;
        OperationInvoiceDetyail.PackingReference = DeliveryDetailGrid._options.dataSource[h].PackingReference;
        OperationInvoiceDetyail.NoOfPallet = DeliveryDetailGrid._options.dataSource[h].NoOfPallet;
        OperationInvoiceDetyail.PONo = DeliveryDetailGrid._options.dataSource[h].PONo;
        OperationInvoiceDetyail.PODate = DeliveryDetailGrid._options.dataSource[h].PODate;
        OperationInvoiceDetyail.IssueOuterCarton = Number(DeliveryDetailGrid._options.dataSource[h].IssueOuterCarton);
        OperationInvoiceDetyail.InnerCarton = Number(DeliveryDetailGrid._options.dataSource[h].InnerCarton);
        OperationInvoiceDetyail.NetWeight = Number(DeliveryDetailGrid._options.dataSource[h].NetWeight);
        OperationInvoiceDetyail.GrossWeight = Number(DeliveryDetailGrid._options.dataSource[h].GrossWeight);
...
$("#EditCHButton").click(function () {
    ...
    $("#DivNotifyParty2").dxRadioGroup({ value: GetProcessSelectedListData[0].NotifyParty2Caption });
    ...
    document.getElementById("TxtDistance").value = GetProcessSelectedListData[0].TotalDistance;
...
function GetConsignee(CID) {
    $.ajax({
        ...
        success: function (results) {
            ...
            res = res.slice(0, -1);
            var RES1 = JSON.parse(res.toString());
            $("#TxtConsigneeID").dxSelectBox({ items: RES1 });
...
$("#BtnGenerateJSON").click(function () {
    var TxtPOID = Number(document.getElementById("TxtCHDID").value);
    if (TxtPOID <= 0) {
        swal("Invoice Not Selected", "Please select invoice first..!", "warning");
        return false;
    }

    let objJSON = [];
    $("#LoadIndicator").dxLoadPanel("instance").option("visible", true);
    for (var i = 0; i < GetProcessSelectedListData.length; i++) {
        $.ajax({
            type: "POST",
            async: false,
            url: "WebService_InvoiceEntry.asmx/GenerateJsonFile",
            data: '{TxtCHDID:' + JSON.stringify(GetProcessSelectedListData[i].InvoiceTransactionID) + '}',
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            success: function (results) {
                var res = results.d.replace(/\\/g, '');
                ...
                var ArrJson = {};
                ArrJson.Version = "1.1";
                ArrJson.TranDtls = RES1.TranDtls[0];
                ArrJson.DocDtls = RES1.DocDtls[0];
                ArrJson.SellerDtls = RES1.SellerDtls[0];
                ArrJson.BuyerDtls = RES1.BuyerDtls[0];
                if (RES1.ShippedDtls.length > 0) ArrJson.ShippedDtls = RES1.ShippedDtls[0];
                ArrJson.ValDtls = RES1.ValDtls[0];
                ArrJson.RefDtls = RES1.RefDtls[0];
                ArrJson.ItemList = RES1.ItemList;
                objJSON.push(ArrJson);
    ...
    if (objJSON.length > 0) {
        var strObj = JSON.stringify(objJSON);

        var data = "text/json;charset=utf-8," + encodeURIComponent(strObj);
        var a = document.createElement('a');
        a.href = 'data:' + data;
        a.download = 'E-Invoice_V1_JSON.json';
        a.click();
        a.remove();

        //this.href = 'data:' + data;
        //this.download = 'E-Invoice_V1_JSON.json';
    }
});

$("#BtnUpdateIRN").click(function () {
    var TxtPOID = Number(document.getElementById("TxtCHDID").value);
    if (TxtPOID === 0 || GetProcessSelectedListData.length <= 0) {
        swal("Invoice Not Selected", "Please select invoice first..!", "warning");
        return false;
    }
    $("#DivEwayDetails").addClass("hidden");
    $("#DivIRNDetails").removeClass("hidden");
    document.getElementById("TxtInvoiceNo").value = GetProcessSelectedListData[0].InvoiceNo;
    document.getElementById("TxtInvoiceDate").value = GetProcessSelectedListData[0].InvoiceDate;
    document.getElementById("TxtInvoiceRefNo").value = "";
    document.getElementById("BtnUpdateIRN").setAttribute("data-toggle", "modal");
    document.getElementById("BtnUpdateIRN").setAttribute("data-target", "#modalEwayDetails");
});
...
$("#BtnUpdateEWay").click(function () {
    ...
    $("#DivEwayDetails").removeClass("hidden");
    $("#DivIRNDetails").addClass("hidden");

    $("#DTEWayBillDate1").dxDateBox({ value: GetProcessSelectedListData[0].EWayBillDate });
    document.getElementById("TxtEWayBillNumber1").value = GetProcessSelectedListData[0].EWayBillNumber;
...
$("#BtnUpdateDetails").click(function () {
    ...
    var TxtPOID = Number(document.getElementById("TxtCHDID").value);
    if (TxtPOID === 0 || GetProcessSelectedListData.length <= 0) {
        swal("Invoice Not Selected", "Please select invoice again..!", "warning");
        return false;
    }
    if ($("#DivEwayDetails").hasClass("hidden")) {
        var TxtInvoiceRefNo = document.getElementById("TxtInvoiceRefNo").value;
        if (TxtInvoiceRefNo === "" || TxtInvoiceRefNo === null) {
            swal("Empty Field..", "Please enter invoice reference no..!", "warning");
            return false;
        }
        ArrayData.InvoiceReferenceNo = TxtInvoiceRefNo;
    } else {
        var TxtEWayBillNumber = document.getElementById("TxtEWayBillNumber1").value;
        if (TxtEWayBillNumber === "" || TxtEWayBillNumber === null) {
            swal("Empty Field..", "Please enter E-Way bill no..!", "warning");
            return false;
        }
        var DtEWayBillDate1 = $("#DtEWayBillDate1").dxDateBox('instance').option('value');
        if (DtEWayBillDate1 === "" || DtEWayBillDate1=== null) {
            swal("Empty Field..", "Please select E-Way bill date..!", "warning");
            return false;
        }
        ArrayData.EWayBillNumber = TxtEWayBillNumber;
        ArrayData.EWayBillDate = DtEWayBillDate1;
    }
    ObjData.push(ArrayData);

    $("#LoadIndicator").dxLoadPanel("instance").option("visible", true);
    $.ajax({
        type: "POST",
        url: "WebService_InvoiceEntry.asmx/UpdateInvoiceDetails",
        data: '{TxtInvoceTransactionID:' + document.getElementById("TxtCHDID").value + ',jsonObjectsInvoicemain:' + JSON.stringify(ObjData) + '}',
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        success: function (results) {
            var res = JSON.stringify(results);
...
function ProcessExcel(data) {
    ...
    //Add filter variable.
    var invoiceNo = GetProcessSelectedListData[0].MaxVoucherNo;
    var invoiceDate = new Date(GetProcessSelectedListData[0].InvoiceDate).format("dd/MM/yyyy");

    //Add the IRN No from Excel file.
    for (var i = 0; i < excelRows.length; i++) {
        try {
            if (Number(excelRows[i]["Doc No"]) === invoiceNo && invoiceDate === new Date(excelRows[i]["Doc Date"]).format("dd/MM/yyyy")) {
                document.getElementById("TxtInvoiceRefNo").value = excelRows[i]["IRN"];
                return;
            }
        } catch (e) {
            console.log(e);
