...
Partial Class ProjectQuotationReportViewer
    ...
    Protected Sub Page_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            Dim DTDetail As DataTable = GetDataTable("Select dbo.CreateOperationName(PQC.ProcessIdStr) as Process,PQC.ProductDescription,nullif(PQC.PackagingDetails,'') PackagingDetails,nullif(PQC.DescriptionOther,'') DescriptionOther, PCM.ProductName,PQC.GSTPercantage,PQC.Quantity,PQC.Rate,PQC.Amount,PQC.UnitCost,PQC.FinalAmount AS TotalAmount,Isnull(PQ.FreightAmount,0) as FreightAmount,PHM.HSNCode as HSN,Round(PQC.GSTAmount,2) as  GST,Round(PQC.CGSTAmount,2) as  CGST,Round(PQC.SGSTAmount,2) as  SGST ,Round(PQC.IGSTAmount,2) as  IGST from ProductQuotationContents as PQC inner join  ProductCatalogMaster as PCM on PCM.ProductCatalogID = PQC.ProductCatalogID inner Join ProductQuotation as PQ on PQ.ProductEstimateID = PQC.ProductEstimateID inner join ProductHSNMaster as PHM on PHM.ProductHSNID = PQC.ProductHSNID  where PQ.CompanyID = " & GBLCompanyID & " and PQ.ProductEstimateID = " & ID)
