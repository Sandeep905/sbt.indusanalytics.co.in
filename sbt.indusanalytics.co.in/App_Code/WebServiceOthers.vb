...
Public Class WebServiceOthers
    ...
    Public Function GetOrderHistory(ByVal OID As String) As String
        Try
            CompanyId = Convert.ToString(HttpContext.Current.Session("CompanyId"))
            Dim Dt As New DataTable
            Str = "Select JOB.OrderBookingID,JOB.SalesOrderNo,Replace(Convert(Nvarchar(30),JOB.OrderBookingDate,106),'','-') As OrderBookingDate,LM.LedgerName as ClientName,Replace(Convert(Nvarchar(30),JOB.PODate,106),'','-') As PODate,SE.LedgerName as SalesPerson from JobOrderBooking as JOB inner join LedgerMaster as LM  on JOB.LedgerID = LM.LedgerID and LM.LedgerGroupID = 1 inner join LedgerMaster as SE on SE.LedgerID = JOB.SalesEmployeeID and SE.LedgerGroupID =  3 where JOB.ProductEstimateID = " & OID & " and Job.CompanyId = " & CompanyId & " Order By JOB.OrderBookingDate,JOB.SalesOrderNo Desc"
            db.FillDataTable(Dt, Str)
            data.Message = db.ConvertDataTableTojSonString(Dt)
            Return js.Serialize(data.Message)
        Catch ex As Exception
