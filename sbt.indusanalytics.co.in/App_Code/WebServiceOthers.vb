...
Public Class WebServiceOthers
            ...
            Else
                If ObjDT.Rows(0)(0) = 0 Then 'Pending For Intenal Appproval
                    If ObjDT.Rows(0)(1) = "Approve" Then
                        Str = "Update JobBooking Set IsInternalApproved=1,InternalApprovedDate=Getdate(),RemarkInternalApproved ='" & ObjDT.Rows(0)(2) & "',InternalApprovedUserID=" & UserId & " Where BookingID=" & ObjDT.Rows(0)(3) & " And CompanyID=" & CompanyId
                    ElseIf ObjDT.Rows(0)(1) = "Rework" Then
                        Str = "Update JobBooking Set IsRework =1 ,ReworkDate = Getdate(), ReworkRemark = '" & ObjDT.Rows(0)(2) & "' ,ReworkBy=" & UserId & " Where BookingID =" & ObjDT.Rows(0)(3) & "  AND CompanyId = " & CompanyId
                    ElseIf ObjDT.Rows(0)(1) = "Reject" Then
                        Str = "Update JobBooking Set IsInternalApproved=0,InternalApprovedDate=Getdate(),RemarkInternalApproved ='',IsCancelled =1 ,CancelledDate = Getdate(), CancelledRemark = '" & ObjDT.Rows(0)(2) & "',CancelledBy=" & UserId & " Where BookingID =" & ObjDT.Rows(0)(3) & "  AND CompanyId = " & CompanyId
                    End If
                ElseIf ObjDT.Rows(0)(0) = 1 Then 'Intenal Appproved
                    db.FillDataTable(DTCheck, "Select Isnull(IsSendForPriceApproval,0) As IsSendForPriceApproval From JobBooking Where BookingID =" & ObjDT.Rows(0)(3) & " AND CompanyId = " & CompanyId)
                    ...
                    If ObjDT.Rows(0)(1) = "Rework" Then
                        Str = "Update JobBooking Set IsInternalApproved=0,InternalApprovedDate=Getdate(),RemarkInternalApproved ='',IsRework =1 ,ReworkDate = Getdate(), ReworkRemark = '" & ObjDT.Rows(0)(2) & "',ReworkBy=" & UserId & " Where BookingID=" & ObjDT.Rows(0)(3) & " And CompanyID=" & CompanyId
                    ElseIf ObjDT.Rows(0)(1) = "Reject" Then
                        Str = "Update JobBooking Set IsInternalApproved=0,InternalApprovedDate=Getdate(),RemarkInternalApproved ='',IsCancelled =1 ,CancelledDate = Getdate(), CancelledRemark = '" & ObjDT.Rows(0)(2) & "',CancelledBy=" & UserId & " Where BookingID =" & ObjDT.Rows(0)(3) & "  AND CompanyId = " & CompanyId
                    End If

                ElseIf ObjDT.Rows(0)(0) = 2 Then 'Rework
                    db.FillDataTable(DTCheck, "Select Max(BookingID) As MaxBookingID From JobBooking Where CompanyId = " & CompanyId & " AND MaxBookingNo IN(Select Distinct MaxBookingNo From JobBooking Where BookingID =" & ObjDT.Rows(0)(3) & " And CompanyID=" & CompanyId & ")")
                    If DTCheck.Rows.Count > 0 Then
                    ...
                    End If
                    If ObjDT.Rows(0)(1) = "Approve" Then
                        Str = "Update JobBooking Set IsInternalApproved=1,InternalApprovedDate=Getdate(),RemarkInternalApproved ='" & ObjDT.Rows(0)(2) & "',InternalApprovedUserID=" & UserId & ",IsRework =0 ,ReworkDate = Getdate(), ReworkRemark = '' Where BookingID=" & ObjDT.Rows(0)(3) & " And CompanyID=" & CompanyId
                    End If
                    'ElseIf ObjDT.Rows(0)(0) = 3 Then
                    '    If ObjDT.Rows(0)(1) = "Approve" Then
                    '        Str = "Update JobBooking Set IsInternalApproved=1,InternalApprovedDate=Getdate(),RemarkInternalApproved ='" & ObjDT.Rows(0)(2) & "',InternalApprovedUserID=" & UserId & ",IsCancelled =0 ,CancelledDate = Getdate(), CancelledRemark = '' Where BookingID=" & ObjDT.Rows(0)(3) & " And CompanyID=" & CompanyId
                    '    ElseIf ObjDT.Rows(0)(1) = "Rework" Then
                    '        Str = "Update JobBooking Set IsCancelled =0 ,CancelledDate = Getdate(), CancelledRemark = '',IsRework =1 ,ReworkDate = Getdate(), ReworkRemark = '" & ObjDT.Rows(0)(2) & "',ReworkBy=" & UserId & " Where BookingID=" & ObjDT.Rows(0)(3) & " And CompanyID=" & CompanyId
                    '    End If
                End If
            End If
    ...
    <WebMethod(EnableSession:=True)>
    <ScriptMethod(ResponseFormat:=ResponseFormat.Json)>
    Public Sub CostApprovalQuoteLoad()
        Try
            CompanyId = Convert.ToString(HttpContext.Current.Session("CompanyId"))
    ...
    Public Function SaveCostApprovalData(ByVal CAData As Object, ByVal BKID As Integer, ByVal Flag As String) As String
        ...
        Try
            ...
            Else
                ...
                TableName = "JobApprovedCost"
                AddColName = "CompanyId, UserId,ApprovedBy,FYear,BookingId,ApprovalNo,MaxApprovalNo,ModifiedDate"     'CompanyId, UserId, FYear,
                ...
                Str = "UPDATE JOBBooking SET ISAPPROVED = 1  WHERE BookingID = '" & BKID & "' And CompanyId = " & CompanyId & " "
        ...
        Try
            ...
            Else
                ...
                If IsNumeric(SalesOrderID) = True And SalesOrderID > 0 Then
                    ...
                    db.ExecuteNonSQLQuery("Update ProductMaster Set IsBooked=1 Where ProductMasterCode IN (" & PMIDS & ")")
                    Key = "Success"
                Else
                    Return SalesOrderID
                End If
