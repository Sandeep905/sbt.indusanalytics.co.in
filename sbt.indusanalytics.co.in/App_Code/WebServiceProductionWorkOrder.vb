...
Public Class WebServiceProductionWorkOrder
    ...
    Public Function LoadLedgerConsignee(ByVal ClientID As Integer) As String
        Try
            ...
            CompanyId = Convert.ToString(HttpContext.Current.Session("CompanyId"))
            str = "Select nullif(LedgerID,'') As LedgerConsigneeID,nullif(LedgerName,'') As LedgerConsigneeName From LedgerMaster Where LedgerGroupID=4 And LedgerID In (Select LedgerID From LedgerMasterDetails Where ParentFieldName='RefClientID' And ParentFieldValue=" & ClientID & " And Isnull(IsDeletedTransaction,0)<>1 And CompanyId = " & CompanyId & ") And Isnull(IsDeletedTransaction,0)<>1 And CompanyId = " & CompanyId
            db.FillDataTable(dataTable, str)
            data.Message = db.ConvertDataTableTojSonString(dataTable)
            Return js.Serialize(data.Message)
        Catch ex As Exception
