...
m:
        ...
        Public Function SendEmails(ByVal subject As String, ByVal body As String, ByVal toUser As String, Optional ByVal attachmentPath As String = Nothing) As String
            ...
            FillDataTable(DtUser, "SELECT Distinct IsnuLL(Nullif(EmailID,''),smtpUserName) As smtpUserID,  Isnull(smtpUserPassword,'') As smtpUserPassword,  Isnull(smtpServer,'smtp.gmail.com') As smtpServer,  Isnull(smtpServerPort,'587') As smtpServerPort,  Isnull(smtpAuthenticate,'True') As smtpAuthenticate,  Isnull(smtpUseSSL,'True') As smtpUseSSL FROM UserMaster Where Isnull(IsBlocked,0)=0 And IsnuLL(IsHidden,0)=0  And CompanyID= " & companyId & "  And UserID= " & userId & "")
            If DtUser.Rows.Count <= 0 Then Return "Invalid user details"
            ...
            For Each email As String In emailAddresses
                If Not validEmailRegex.IsMatch(email.Trim()) Then
                    Return "Invalid email address: " & email
                End If
            Next

            Dim message As MailMessage = New MailMessage(
                DtUser.Rows(0)("smtpUserID").ToString(),
                emailAddresses(0)) With {
            ...
            For i As Integer = 1 To emailAddresses.Length - 1
                Dim emailAddress As String = emailAddresses(i).Trim()
                If Not String.IsNullOrEmpty(emailAddress) AndAlso validEmailRegex.IsMatch(emailAddress) Then
